...
test('visitor message reopens a closed conversation', function (): void {
    ...
    if (DB::getDriverName() === 'pgsql') {
        ...
        $siteLock = $queries->search(fn (string $query, int $index): bool => $index > $accountLock
            && str_contains($query, 'from "sites"')
            && str_contains($query, 'for share'));
...
test('visitor can report cobrowse page state for their active session', function (): void {
    ...
    $response
        ...
        ->assertJsonPath('data.cobrowse.status', 'granted')
        ->assertJsonPath('data.page_state.page_url', 'https://docs.example.test/install')
    ...
    expect($session->fresh()->metadata['page_state'])
        ->page_url->toBe('https://docs.example.test/install')
...
test('visitor can report a cobrowse snapshot for their active session', function (): void {
    ...
    $response
        ...
        ->assertJsonPath('data.payload_budget.widget_mutation_queue_max_records', 250)
        ->assertJsonPath('data.payload_budget.widget_resync_max_attempts', 3)
        ->assertJsonPath('data.snapshot.page_url', 'https://docs.example.test/install')
    ...
    expect($session->fresh()->metadata)
        ...
        ->snapshot->page_url->toBe('https://docs.example.test/install')
...
test('visitor can report bounded cobrowse mutation batches for their active session', function (): void {
    ...
    expect($mutations)
        ...
        ->last_page_url->toBe('https://docs.example.test/install')
