...
test('the alert center renders its relative times in the agent language', function (): void {
    // Carbon reads the application locale independently of the catalogue. A
    // translated shell is not enough: the request locale must reach timestamps
    // generated by the notification card too.
    $agent = languageAgent('de');
    $site = Site::factory()->for($agent->account)->create(['name' => 'Datenpunkt Hilfe']);
    $visitor = Visitor::factory()->for($site)->create(['anonymous_id' => 'datenpunkt-zeit']);
    $conversation = Conversation::factory()->for($site)->for($visitor)->create([
        'assigned_agent_id' => $agent->id,
        'support_code' => 'WF-ZEIT1',
        'subject' => 'Datenpunkt Zeitfrage',
    ]);
    $message = ConversationMessage::factory()->for($conversation)->create([
        'sender_type' => Visitor::class,
        'sender_id' => $visitor->id,
        'body' => 'Datenpunkt wartet auf eine Antwort.',
    ]);

    $agent->notify(new ConversationNeedsReply($message));
    $notification = $agent->notifications()->firstOrFail();
    $notification->forceFill(['created_at' => now()->subHours(3)])->save();

    App::setLocale('de');
    $inGerman = $notification->fresh()->created_at->diffForHumans();

    App::setLocale('en');
    $inEnglish = $notification->fresh()->created_at->diffForHumans();
...
test('the whole document says one language, because the shell speaks it too', function (): void {
    ...
    $this->actingAs($agent)
        ...
        ->assertSee('<html lang="de"', false)
        ->assertSee('Abmelden')
        ->assertSee('Benachrichtigungszentrale')
...
test('the title and the breadcrumb are translated with everything else', function (): void {
    ...
    // Alerts now belong to the extracted dashboard surface too.
    $this->actingAs($agent)
        ->get(route('dashboard.alerts.index'))
        ->assertOk()
        ->assertSee('<title>Benachrichtigungen</title>', false)
        ->assertSee('<span class="wf-crumb-current">Benachrichtigungen</span>', false);
});
