            ...
            <div>
                <p class="text-sm text-gray-700 leading-5 dark:text-gray-600">
                    {!! __('pagination.summary', [
                        'first' => '<span class="font-medium">'.e(\App\Support\ReaderNumber::count($paginator->firstItem() ?? 0)).'</span>',
                        'last' => '<span class="font-medium">'.e(\App\Support\ReaderNumber::count($paginator->lastItem() ?? 0)).'</span>',
                        'total' => '<span class="font-medium">'.e(\App\Support\ReaderNumber::count($paginator->total())).'</span>',
                    ]) !!}
                </p>
            </div>
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-gray-200 border border-gray-300 cursor-default leading-5 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300">{{ \App\Support\ReaderNumber::count($page) }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-700 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-300 dark:hover:text-gray-300 dark:active:bg-gray-700 dark:focus:border-blue-800 hover:bg-gray-100 dark:hover:bg-gray-900" aria-label="{{ __('pagination.go_to_page', ['page' => \App\Support\ReaderNumber::count($page)]) }}">
                                        {{ \App\Support\ReaderNumber::count($page) }}
                                    </a>
                                @endif
                            @endforeach
                        @endif
