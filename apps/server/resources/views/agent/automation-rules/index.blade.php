...
<x-layouts.account :title="__('automation_rules.title')">
    ...
    {{-- Run order is stated where it is set, on the rule form, so this keeps
         only what holds for the rules and macros tabs below. --}}
    <x-details-disclosure class="automation-safety" :summary="__('automation_rules.safety.heading')">
    ...
    <x-tabs
        ...
        :label="__('automation_rules.tabs.label')"
        :tabs="[
            ['id' => 'rules', 'label' => __('automation_rules.tabs.rules')],
            ['id' => 'macros', 'label' => __('automation_rules.tabs.macros')],
            ['id' => 'proactive', 'label' => __('automation_rules.tabs.proactive')],
            ['id' => 'executions', 'label' => __('automation_rules.tabs.executions')],
        ]"
    >
        <x-tab-panel id="rules" active>
            ...
            <section class="section" aria-labelledby="proactive-sites-heading">
                <div class="section-header">
                    <div>
                        ...
                        <p class="lede">{{ __('proactive_messages.account.lede') }}</p>
                    </div>
                </div>
            ...
            <section class="section" aria-labelledby="automation-executions-heading">
                <div class="section-header">
                    <div>
                        ...
                        <p class="lede">{{ __('automation_rules.executions.lede') }}</p>
                    </div>
                </div>
                ...
                @else
                    ...
                    {{-- A page link has to reopen the log, including where the
                         tab memory is unavailable (private windows). --}}
                    {{ $executions->fragment('tab-executions')->links() }}
                @endif
            </section>
        </x-tab-panel>
    </x-tabs>
