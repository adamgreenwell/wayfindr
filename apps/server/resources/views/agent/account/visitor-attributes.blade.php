    ...
    <section class="section" aria-labelledby="defined-attributes-heading">
        ...
        @if ($definitions->isEmpty())
            <div class="empty empty-state">
                <strong>{{ __('visitor_attributes.existing.empty') }}</strong>
                <div class="empty-state-actions">
                    <a class="button secondary" href="#create-attribute-heading">{{ __('visitor_attributes.existing.empty_action') }}</a>
                </div>
            </div>
        @else
            @foreach ($definitions as $definition)
                {{-- One item in this card, not a card of its own: .section here
                     ...
                     with Delete flush against the inner card's edge. --}}
                <article class="section-item" id="attribute-{{ $definition->id }}">
                    <div class="section-header">
                    ...
                    <div class="section-item-body">
                        <form method="POST" action="{{ route('dashboard.account.visitor-attributes.update', $definition) }}" aria-labelledby="attribute-{{ $definition->id }}-heading">
                            @csrf
                            @method('PUT')
