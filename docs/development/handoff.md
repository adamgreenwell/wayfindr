...
| **Cobrowse consent audit** | Granting, declining or revoking writes `cobrowse.consent_granted` / `_declined` / `_revoked`, keyed on the status changing, so a repeat or racing answer is not a fresh consent (the 5s loop only GETs status; it never posts an answer). Grants are audited inside the write transaction; declines and revocations after it commits, best-effort (the audit failure is reported, never raised, so a stop always takes effect). The row records **the answer — which way, and when — not the provenance of the session that sent it**; if you need "did the session that consented also deliver the screen", that comparison does not exist yet. | `app/Support/CobrowseAuditTrail.php` (`consentAnswered`), `docs/privacy/cobrowse-data-boundaries.md` | #1000 |
...
- **Test toolchain**: server tests need **PHP >= 8.4.1** (`composer.json`
  ...
  `php artisan config:clear` (same binary, same `apps/server` directory)
  **before** Pest: a cached `bootstrap/cache/config.php`
