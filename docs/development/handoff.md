...
| **Cobrowse consent audit** | Granting, declining or revoking writes `cobrowse.consent_granted` / `_declined` / `_revoked`, keyed on the status changing rather than on each 5s poll. Grants are audited inside the write transaction; declines and revocations after it commits. The row records **the answer — which way, and when — not the provenance of the session that sent it**; if you need "did the session that consented also deliver the screen", that comparison does not exist yet. | `app/Support/CobrowseAuditTrail.php` (`consentAnswered`), `docs/privacy/cobrowse-data-boundaries.md` | #1000 |
