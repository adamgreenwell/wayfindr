...
- A visitor-facing autonomous answer agent remains deferred by ADR 0004 and
  ...
  [#997](https://github.com/adamgreenwell/wayfindr/pull/997) later rotated the
  suite identity after changing a fixture key, so those captures are historical
  and cannot be compared with a new run under the current identity. A comparable
  baseline requires recapturing every proposed route before #762 can use
  same-route evidence after meaningful elapsed time or a model revision to
  revisit ADR 0004. All four September captures were recorded within about 78
  minutes, so this is point-in-time
