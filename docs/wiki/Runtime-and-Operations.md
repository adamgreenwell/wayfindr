...
- **The ticket queue is capped in `v0.9.0`.** Its attention and external-issue
  ...
  stops counting. Older public `v0.7.0` still hydrates every matching ticket:
  its measurement needs a 1 GB PHP memory limit while that image sets 256M, so
  a large desk on that release can exhaust memory.
