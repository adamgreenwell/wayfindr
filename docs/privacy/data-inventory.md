...
| Grounded-answer evaluation fixtures | Synthetic support questions, synthetic published-article text, expected citations/fact phrases, refusal expectations, and recorded synthetic baseline answers | `resources/evaluations/grounded-answers/*.json` | Public, versioned development evidence only; it is not runtime support data and the offline command does not call a provider or database. Reports contain aggregate metrics plus case IDs/reason codes, never questions, articles, or answers. Private customer transcripts and evaluation datasets with real user information must not be committed. Operators using a private local fixture file own its access and retention. |
