...
| Grounded-answer evaluation fixtures and private runs | Synthetic support questions, synthetic published-article text, expected citations/fact phrases/refusal reasons, curated baseline answers/confidence, or a private provider/model-tagged candidate run | Public baseline: `resources/evaluations/grounded-answers/*.json`; captured runs: operator-selected path outside the repository | Public, versioned development evidence only; it is not runtime support data and offline scoring does not call a provider or database. Explicit capture sends fixture questions/articles—but never expected answers—to the configured provider and writes a new `0600` file outside the checkout. Reports contain run metadata, aggregate metrics, and case IDs/reason codes, never questions, articles, or answers. Private customer transcripts, provider runs, and evaluation datasets with real user information must not be committed. Operators own private-run access and retention. |
