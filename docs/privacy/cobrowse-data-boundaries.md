...
An answer is bound to the request it was shown for. A visitor's agreement
applies to that one request and nothing else, so if an agent ends a session and
asks again, the new request is unanswered until the visitor answers it. Stopping
is never bound this way: a visitor can always end a session they are in.
...
Answering the consent prompt is recorded too. Granting, declining or revoking
writes a `cobrowse.consent_granted`, `cobrowse.consent_declined` or
`cobrowse.consent_revoked` audit event, subject to the cobrowse session and
carrying provenance only: the support code, the status before and after, and
the consent and end timestamps. The entry keys on the status actually changing,
so a repeated or racing answer — a double-tapped button, a retry, two tabs — is
not logged as a fresh consent. The widget's five-second loop cannot reach this
at all: it polls status with a GET, and only the visitor's own allow or decline
posts an answer.

A grant is written inside the same transaction as the status, so sharing cannot
begin on a record that failed to persist. A decline or revocation is written
after that transaction commits, and a failure there is reported rather than
raised: rolling a refusal back over its own audit row would leave the session
shared while telling the visitor it had stopped. Recording a refusal is
therefore best-effort by design — the stop itself takes effect either way, even
in the rare case where the row recording it does not.

What the row establishes is that the prompt was answered, which way, and when.
It is a record of the answer, not proof of who produced it.
