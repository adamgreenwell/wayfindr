...
- Automation rules belong to one account, default to disabled, select a typed event, and keep ordered condition and action lists as JSON. Equal positions are evaluated by row ID so order remains deterministic while an operator is reordering rules. Conditions use a typed field/operator/value vocabulary and actions are limited to assignment, labels, priority, status, agent notification, and private ticket notes; no action in this contract can send a visitor message. Visitor-message rules run before the reply alert is chosen, so they may assign or reprioritise the conversation but may not close it: the definition refuses that combination, and a rule saved before the refusal still runs its other actions while the close is recorded in the execution log as skipped. Conversation-created rules may close a conversation nobody is waiting in yet -- the widget and the API open it empty -- but the close is withheld and logged the same way when the conversation already holds a visitor message, as an emailed conversation does when its creation rules run.
