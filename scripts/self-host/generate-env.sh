...
# This compose stack mounts one storage volume across every app service, so the
# file maintenance-mode marker is visible to the web process — a precondition for
# the operator's in-GUI restore. Do not set this true on a deployment where the
# web and worker do not share storage.
WAYFINDR_RESTORE_FILE_MAINTENANCE_SHARED=true
REDIS_CLIENT=predis
REDIS_HOST=redis
REDIS_PASSWORD=
REDIS_PORT=6379
